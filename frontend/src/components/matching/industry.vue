<template lang="html">
  <div>
    <div class="widget-subhead">
      <h2 class="widget-head">Industry</h2>
      <font-awesome-icon @click="toggleHelpText()" :icon="['fal', 'question-circle']" class="help-icon" :class="{ active : helpTextActive}"/>
    </div>
    <p class="help-text" v-if="helpTextActive">Add information to get insight about how each of your contacts relate to your industry or market: tags (expert, familiar, employed), topics (key concepts that match the contact to the industry), and a match score.</p>
    ...
    <button class="solid-button" @click="addMatch()">add new industry</button>
  </div>
</template>
